...
# loop over images
for i in range(len(config.IMAGE_CATEGORIES)):
	for ind in range(1, 10):
		annot_filename = f'{config.IMAGE_CATEGORIES[i]}_annotation_{str(ind).zfill(4)}.mat'
		img_filename = f'{config.IMAGE_CATEGORIES[i]}_image_{str(ind).zfill(4)}.jpg'
		annots = loadmat(os.path.join(config.ANNOTS_PATH, annot_filename))
		imagePath = os.path.join(config.IMAGES_PATH, img_filename)
...
H = model.fit(x=aug.flow(trainImages, trainLabels, batch_size=config.BATCH_SIZE),
	validation_data=(testImages, testLabels), steps_per_epoch=len(trainImages) // config.BATCH_SIZE,
